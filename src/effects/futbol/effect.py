...
def run(matrix, config):
    ...
    while matrix.ready():
        ...
        # Draw yard line
        matrix.line(
            (0, int(y_max / 2)), (x_max, int(y_max / 2)), matrix.color("white"), 1
        )

        # Draw tick marks (sorry the math is a little messy)
        for tick in range(1, 8):
            matrix.line(
                (int(x_max / 3) - 3, int(y_max / 8 * tick)),
                (int(x_max / 3), int(y_max / 8 * tick)),
                matrix.color("white"),
                1,
            )
            matrix.line(
                (int(x_max - (x_max / 3)), int(y_max / 8 * tick)),
                (int(x_max - (x_max / 3) + 3), int(y_max / 8 * tick)),
                matrix.color("white"),
                1,
            )
